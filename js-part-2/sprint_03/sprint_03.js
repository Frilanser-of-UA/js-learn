...
const t11 = () => {
    arr_11.forEach((item, index) => {
       if(res_11.length < arr_11.length) res_11.push(index);
    });
    return res_11;
};
...
const t12 = () => {
    arr_12.forEach((item, index) => {
        if(res_12.length < arr_12.length && item % 2 == 0) res_12.push(index);
     });
     return res_12;
};
